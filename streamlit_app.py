...
with tab_overview:
    ...
    step_tag("STEP 01 · MODEL PERFORMANCE")
    st.subheader("Model Performance (Reference Validation Cohort)")
    st.caption(
        "Computed on a held-out split of the model's reference training cohort — provided for "
        "transparency into how the underlying classifier behaves, not a claim about this app's "
        "performance on your specific patient population."
    )
    perf_col1, perf_col2 = st.columns(2)
    with perf_col1:
        st.pyplot(style_roc_chart(_fpr, _tpr, _roc_auc))
    with perf_col2:
        st.pyplot(style_pr_chart(_recall, _precision, _avg_precision))
